<?php
	...
	if( !class_exists('Wbcr_FactoryForms401_DropdownControl') ) {

		class Wbcr_FactoryForms401_DropdownControl extends Wbcr_FactoryForms401_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo $ajax_id ?>'] = {
							...
							'emptyList': '<?php echo $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_401') ) ?>'
						};
					</script>
					<?php
			...
			protected function defaultHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('- empty -', 'wbcr_factory_forms_401'));
				...
				foreach((array)$items as $item) {
					...
					if( isset($item['type']) || isset($item['value']) ) {
						...
						$title = isset($item['title'])
							? $item['title']
							: __('- empty -', 'wbcr_factory_forms_401');
